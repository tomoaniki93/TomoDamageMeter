...
#
# Copied from TomoMod (Tools/lint_secret_values.py). Only the CONFIG block
# (SCAN_ROOTS), the checkout check and the --help description differ: keep
# the rest in sync with TomoMod, and regenerate apidoc_secrets.txt there.
#
...
SCAN_ROOTS = ["Core", "Modules", "Config", "Locales"]
...
def main():
    ap = argparse.ArgumentParser(description="Static secret-value audit for TomoDamageMeter.")
    ...
    if not (repo / "TomoDamageMeter.toc").is_file():
        sys.stderr.write("not a TomoDamageMeter checkout: %s\n" % repo)
        return 2
